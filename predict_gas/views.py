...
def supply(request):
    if request.method == 'GET':
        req_dic = cache.get('supply_cache')
        if not req_dic:
            req_dic = {}
            ...
            cache.set('supply_cache', req_dic, 86400)
...
def demand(request):
    if request.method == 'GET':
        req_dic = cache.get('demand_cache')
        if not req_dic:
            req_dic = {}
            ...
            cache.set('demand_cache', req_dic, 86400)
...
def supply3():
    ...
    if not dic:
        ...
        dic['diff'] = diff_list
        cache.set('supply3_cache', dic, 86400)
    return dic
...
def demand3(regionid=1):
    ...
    if not dic:
        ...
        cache.set('demand3_cache', dic, 86400)
...
def percentage():
    dic = cache.get('percentage_cache')
    ...
    if not dic:
        ...
        cache.set('percentage_cache', dic, 86400)

    return dic


def scatter():
    dic = cache.get('scatter_cache')
    ...
    if not dic:
        ...
        cache.set('scatter_cache', dic, 86400)

    return dic


def box():
    dic = cache.get('box_cache')
    ...
    if not dic:
        ...
        cache.set('box_cache', dic, 86400)

    return dic


def demandfactor(request):
    context = cache.get('demandfactor_cache')
    ...
    if not context:
        ...
        context['household_avg2'] = household_avg2
        context['year2'] = year2
        context['demand_avg2'] = demand_avg2
        context['gasproducerprice3'] = gasprice3
        context['oilproducerprice3'] = oilprice3
        context['relativeprice3'] = relativeprice3
        context['year3'] = year3
        context['importprice_list'] = importprice_list
        context['importamount_list'] = importamount_list
        context['importcost_list'] = importcost_list
        context['seoul'] = seoul
        context['pusan'] = pusan
        context['daegu'] = daegu
        context['incheon'] = incheon
        context['gwangju'] = gwangju
        context['daejun'] = daejun
        context['ulsan'] = ulsan
        context['kyeonggi'] = kyeonggi
        context['kangwon'] = kangwon
        context['chungbuk'] = chungbuk
        context['chungnam'] = chungnam
        context['jeonbuk'] = jeonbuk
        context['jeonnam'] = jeonnam
        context['kyeongbuk'] = kyeongbuk
        context['kyeongnam'] = kyeongnam
        context['jeju'] = jeju
        context['sejong'] = sejong
        context['year5'] = year5

        cache.set('demandfactor_cache', context, 86400)

    return render(request, 'demandfactor.html', {'context': context})